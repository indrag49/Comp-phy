...
NFD <- function(X, x, y) {
  ...
  #if(length(f_dashed) < f1) f_dashed <- c(f_dashed, numeric(length(f1)-length(f_dashed)))
  f2 <- c(1, (s-1), (6*s^2-18*s+11)/12, (2*s^3-12*s^2+21*s-10)/12)
  #if(length(f_double_dashed) < f2) f__double_dashed <- c(f_double_dashed, numeric(length(f2)-length(f_double_dashed)))
